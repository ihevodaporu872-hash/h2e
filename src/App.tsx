...
function App() {
  ...
  const analyzeBOQDifferences = (comparisonData: { id: string; fileId: string; label: string; value: number | null; calculationDate: string }[]) => {
    ...
    // Engineering section mapping - hierarchical construction scopes (Moscow standard)
    const sectionAliases: Record<string, string[]> = {
      // ЗЕМЛЯНЫЕ РАБОТЫ
      'ЗЕМЛЯНЫЕ': ['ЗЕМЛЯНЫЕ', 'ЗЕМЛЯ', 'ГРУНТ', 'КОТЛОВАН', 'ВЫЕМКА', 'ВЫВОЗ ГРУНТА', 'РАЗРАБОТКА ГРУНТА'],
      'ОРГАНИЗАЦИЯ': ['ОРГАНИЗАЦИЯ', 'ОСП', 'ВРЕМЕННЫЕ', 'БЫТОВОЙ', 'ОГРАЖДЕНИЕ ПЛОЩАДКИ', 'МОЙКА КОЛЕС'],
      'УСТРОЙСТВО КОТЛОВАНА': ['УСТРОЙСТВО КОТЛОВАНА', 'КОТЛОВАН', 'ШПУНТ', 'РАСПОР', 'СТЕНА В ГРУНТЕ', 'ШПУНТОВОЕ'],
      'ВОДООТВЕДЕНИЕ': ['ВОДООТВЕДЕНИЕ', 'ВОДОПОНИЖЕНИЕ', 'ДРЕНАЖ'],

      // МОНОЛИТНЫЕ РАБОТЫ
      'МОНОЛИТНЫЕ': ['МОНОЛИТНЫЕ', 'МОНОЛИТ', 'БЕТОН', 'ЖЕЛЕЗОБЕТОН', 'ЖБ', 'АРМАТУРА', 'ОПАЛУБКА'],
      'ФУНДАМЕНТ': ['ФУНДАМЕНТ', 'ФУНДАМЕНТНАЯ ПЛИТА', 'ОСНОВАНИЕ'],
      'КОЛОННЫ': ['КОЛОНН', 'ПИЛОН', 'СТОЙК'],
      'БАЛКИ': ['БАЛК', 'РИГЕЛ', 'ПРОГОН'],
      'ПЕРЕКРЫТИЯ': ['ПЕРЕКРЫТИ', 'ПЛИТА ПЕРЕКРЫТИЯ'],
      'ЛЕСТНИЦЫ': ['ЛЕСТНИЦ', 'СТУПЕН'],
      'СТЕНЫ МОНОЛИТ': ['СТЕНА', 'СТЕНЫ'],

      // ФАСАДНЫЕ РАБОТЫ
      'ФАСАДНЫЕ': ['ФАСАДНЫЕ', 'ФАСАД', 'ОБЛИЦОВКА', 'НВФ', 'НАВЕСНОЙ ФАСАД'],
      'СВЕТОПРОЗРАЧНЫЕ': ['СВЕТОПРОЗРАЧ', 'ВИТРАЖ', 'ОКНО', 'ОКОНН', 'АЛЮМИНИЕВ', 'СТЕКЛОПАКЕТ'],
      'СТОЙКА-РИГЕЛЬ': ['СТОЙКА-РИГЕЛЬ', 'ПРОФИЛЬ ФАСАД'],

      // КРОВЕЛЬНЫЕ РАБОТЫ
      'КРОВЛЯ': ['КРОВЛЯ', 'КРОВЕЛЬНЫЕ', 'КРЫША', 'МЕМБРАНА', 'РУЛОННАЯ'],

      // ОТДЕЛОЧНЫЕ РАБОТЫ
      'ОТДЕЛОЧНЫЕ': ['ОТДЕЛОЧНЫЕ', 'ОТДЕЛКА'],
      'ШТУКАТУРКА': ['ШТУКАТУР', 'ШПАКЛЕВ'],
      'ПОКРАСКА': ['ПОКРАСКА', 'ОКРАСКА', 'КРАСКА', 'МАЛЯРН'],
      'ПЛИТКА': ['ПЛИТК', 'КЕРАМОГРАНИТ', 'КАФЕЛЬ'],
      'ПОЛЫ': ['ПОЛ', 'НАПОЛЬН', 'СТЯЖКА', 'НАЛИВНОЙ'],
      'ПОТОЛКИ': ['ПОТОЛОК', 'ПОТОЛОЧН', 'ПОДВЕСНОЙ'],

      // КЛАДОЧНЫЕ РАБОТЫ
      'КЛАДОЧНЫЕ': ['КЛАДОЧНЫЕ', 'КЛАДКА', 'КИРПИЧ', 'БЛОК', 'ГАЗОБЕТОН', 'ПЕНОБЛОК'],

      // ВИС / МЕХАНИЧЕСКИЕ СИСТЕМЫ
      'ВИС': ['ВИС', 'МЕХАНИЧЕСКИЕ', 'ИНЖЕНЕРНЫЕ СИСТЕМЫ'],
      'ВЕНТИЛЯЦИЯ': ['ВЕНТИЛЯЦ', 'ВОЗДУХОВОД', 'ДИФФУЗОР', 'ПРИТОЧ', 'ВЫТЯЖ'],
      'ВОДОСНАБЖЕНИЕ': ['ВОДОСНАБЖ', 'ХВС', 'ГВС', 'ВОДОПРОВОД', 'СМЕСИТЕЛЬ'],
      'КАНАЛИЗАЦИЯ': ['КАНАЛИЗАЦ', 'САНТЕХПРИБОР', 'УНИТАЗ', 'РАКОВИН'],
      'ОТОПЛЕНИЕ': ['ОТОПЛЕН', 'РАДИАТОР', 'КОНВЕКТОР', 'ТЕПЛЫЙ ПОЛ'],
      'ПОЖАРОТУШЕНИЕ': ['ПОЖАРОТУШ', 'СПРИНКЛЕР', 'АУПТ', 'ДРЕНЧЕР'],
      'ДЫМОУДАЛЕНИЕ': ['ДЫМОУДАЛЕН', 'ПРОТИВОДЫМ'],
      'ИТП': ['ИТП', 'ТЕПЛОВОЙ ПУНКТ'],

      // ВИС / ЭЛЕКТРИЧЕСКИЕ СИСТЕМЫ
      'ЭЛЕКТРИКА': ['ЭЛЕКТРИК', 'ЭЛЕКТРОМОНТАЖ', 'ЭЛЕКТРОСНАБЖ'],
      'КАБЕЛИ': ['КАБЕЛ', 'ПРОВОД', 'СИЛОВОЙ КАБЕЛЬ'],
      'ЛОТКИ': ['ЛОТОК', 'КАБЕЛЬНЫЙ ЛОТОК', 'КОРОБ'],
      'ЩИТОВОЕ': ['ЩИТ', 'ВРУ', 'ГРЩ', 'ЩИТОВОЕ', 'ЭЛЕКТРОЩИТ'],
      'ОСВЕЩЕНИЕ': ['ОСВЕЩЕН', 'СВЕТИЛЬНИК', 'ЛАМПА', 'ЛЮСТРА'],
      'ЗАЗЕМЛЕНИЕ': ['ЗАЗЕМЛЕН', 'МОЛНИЕЗАЩИТ'],

      // СЛАБОТОЧНЫЕ СИСТЕМЫ
      'СЛАБОТОЧНЫЕ': ['СЛАБОТОЧ', 'СКС', 'ВИДЕОНАБЛЮД', 'СКУД', 'ДОМОФОН'],
      'ПОЖАРНАЯ СИГНАЛИЗАЦИЯ': ['ПОЖАРН СИГНАЛ', 'АПС', 'ИЗВЕЩАТЕЛЬ'],
      'ОПОВЕЩЕНИЕ': ['ОПОВЕЩЕН', 'СОУЭ'],

      // БЛАГОУСТРОЙСТВО
      'БЛАГОУСТРОЙСТВО': ['БЛАГОУСТРОЙСТВО', 'ОЗЕЛЕНЕН', 'ГАЗОН', 'АСФАЛЬТ', 'ТРОТУАР', 'БРУСЧАТКА'],
      'ДЕРЕВЬЯ': ['ДЕРЕВ', 'ПОСАДКА', 'САЖЕНЦ', 'КУСТАРНИК'],
      'МАФ': ['МАФ', 'МАЛЫЕ ФОРМЫ', 'СКАМЕЙ', 'УРНА'],
      'ВОДООТВОДНЫЕ ЛОТКИ': ['ВОДООТВОД', 'ЛИВНЕВКА', 'ДОЖДЕПРИЕМНИК'],

      // ДВЕРИ, ЛЮКИ, ВОРОТА
      'ДВЕРИ': ['ДВЕРЬ', 'ДВЕРН', 'ДВЕРНОЙ'],
      'ПРОТИВОПОЖАРНЫЕ ДВЕРИ': ['ПРОТИВОПОЖАРН', 'EI60', 'EI30', 'ОГНЕСТОЙК'],
      'ЛЮКИ': ['ЛЮК', 'ЛЮКОВ'],
      'ВОРОТА': ['ВОРОТА', 'РОЛЕТ', 'СЕКЦИОНН'],

      // ЛИФТОВОЕ ОБОРУДОВАНИЕ
      'ЛИФТЫ': ['ЛИФТ', 'ЭСКАЛАТОР', 'ПОДЪЕМНИК'],

      // МЕТАЛЛОКОНСТРУКЦИИ
      'МЕТАЛЛИЧЕСКИЕ': ['МЕТАЛЛИЧЕСКИЕ', 'МЕТАЛЛОКОНСТРУКЦИИ', 'МК', 'СТАЛЬ', 'СТАЛЬН'],

      // ГИДРОИЗОЛЯЦИЯ
      'ГИДРОИЗОЛЯЦИЯ': ['ГИДРОИЗОЛЯЦИЯ', 'ГИДРОИЗОЛЯЦИОННЫЕ', 'БИТУМ', 'МАСТИКА'],
    };
    ...
    const catItems = (items: BOQItem[]) => ({
      materials: items.filter(i => i.elementType.toLowerCase().includes('мат')),
      works: items.filter(i => i.elementType.toLowerCase().includes('раб'))
    });
    const cat1 = catItems(final1);
    const cat2 = catItems(final2);
    ...
    // Generate engineering comment based on change type
    const getEngineeringComment = (change: ChangeType, elementType: string, priceDiff: number, qtyDiff: number): string => {
      const isMat = elementType.toLowerCase().includes('мат');
      switch (change) {
        case 'new':
          return isMat ? 'Добавлен новый материал. Проверить соответствие проекту и спецификации.'
                       : 'Добавлена новая работа. Уточнить технологию и объём у проектировщика.';
        case 'removed':
          return isMat ? 'Материал исключён из ВОР. Проверить актуальность проектных решений.'
                       : 'Работа исключена. Возможно изменение технологии или объединение позиций.';
        case 'price_change':
          return priceDiff > 0
            ? (isMat ? 'Рост цены материала. Возможные причины: изменение поставщика, рост рынка, смена марки.'
                     : 'Увеличение расценки. Проверить обоснование в ТКП/КП подрядчика.')
            : (isMat ? 'Снижение цены материала. Возможна оптимизация или замена аналогом.'
                     : 'Снижение расценки. Возможна оптимизация технологии.');
        case 'qty_change':
          return qtyDiff > 0
            ? 'Увеличение объёма. Причины: уточнение по РД, изменение проекта, доп. работы.'
            : 'Уменьшение объёма. Проверить актуальность по чертежам и ведомостям.';
        case 'price_qty_change':
          return 'Комплексное изменение цены и объёма. Требуется детальная проверка по РД.';
        case 'type_change':
          return 'Изменён тип элемента. Проверить корректность классификации.';
        default:
    ...
    map2.forEach((list2, key2) => {
      ...
      if (semanticMatch) {
        ...
        changes.push({
          ...
          workType: item2.constructionCost.substring(0, 20) || workTypeClean.substring(0, 20),
          elementType: item2.elementType,
          v1Price: 0, v1Qty: 0, v1Unit: '-',
          v2Price: item2.pricePerUnit, v2Qty: qty2, v2Unit: item2.unit,
          diff: sum2, changeType: 'new',
    ...
    map1.forEach((list1, key1) => {
      ...
      if (!reverseSemanticMatch) {
        ...
        const qty1 = list1.reduce((s, i) => s + i.quantity, 0);
        changes.push({
          name: item1.name.substring(0, 35),
          workType: item1.constructionCost.substring(0, 20) || workTypeClean.substring(0, 20),
          elementType: item1.elementType,
          v1Price: item1.pricePerUnit, v1Qty: qty1, v1Unit: item1.unit,
          v2Price: 0, v2Qty: 0, v2Unit: '-',
          diff: -sum1, changeType: 'removed',
          reason: 'Позиция удалена из версии 2',
          comment: getEngineeringComment('removed', item1.elementType, 0, 0)
        });
      }
    });
    ...
    const changedCount = changes.filter(c => !['new', 'removed'].includes(c.changeType)).length;

    // Short version labels
    const v1 = boq1.label.replace('Затраты ', '').replace(' Прямые', '').substring(0, 15);
    const v2 = boq2.label.replace('Затраты ', '').replace(' Прямые', '').substring(0, 15);

    // Build HTML
    let html = `<div class="boq-eng-report">`;

    // Header
    html += `<div class="boq-eng-header">`;
    html += `<div class="eng-title">📊 Инженерный анализ BOQ: ${workTypeClean}</div>`;
    html += `<div class="eng-versions">${v1} → ${v2}</div>`;
    html += `</div>`;

    // Summary cards
    html += `<div class="eng-summary-cards">`;
    const matPct = totalMat1 !== 0 ? ((matDiff / totalMat1) * 100).toFixed(1) : '0';
    const workPct = totalWork1 !== 0 ? ((workDiff / totalWork1) * 100).toFixed(1) : '0';
    html += `<div class="eng-card ${matDiff > 0 ? 'up' : matDiff < 0 ? 'down' : ''}">`;
    html += `<div class="card-label">🧱 Материалы</div>`;
    html += `<div class="card-value">${matDiff >= 0 ? '+' : ''}${formatCurrency(matDiff)}</div>`;
    html += `<div class="card-pct">${matDiff >= 0 ? '+' : ''}${matPct}%</div>`;
    html += `</div>`;
    html += `<div class="eng-card ${workDiff > 0 ? 'up' : workDiff < 0 ? 'down' : ''}">`;
    html += `<div class="card-label">🔧 Работы</div>`;
    html += `<div class="card-value">${workDiff >= 0 ? '+' : ''}${formatCurrency(workDiff)}</div>`;
    html += `<div class="card-pct">${workDiff >= 0 ? '+' : ''}${workPct}%</div>`;
    html += `</div>`;
    html += `<div class="eng-card total ${totalDiff > 0 ? 'up' : totalDiff < 0 ? 'down' : ''}">`;
    html += `<div class="card-label">📋 Итого</div>`;
    html += `<div class="card-value">${totalDiff >= 0 ? '+' : ''}${formatCurrency(totalDiff)}</div>`;
    html += `<div class="card-badges">`;
    if (newCount > 0) html += `<span class="badge new">+${newCount}</span>`;
    if (removedCount > 0) html += `<span class="badge rem">-${removedCount}</span>`;
    ...
    if (changes.length > 0) {
      ...
      changes.forEach(c => {
        ...
        // Short type label: суб-мат, суб-раб, мат, раб
        const typeLower = c.elementType.toLowerCase();
              ...
              <div className="agent-panel" style={{ marginTop: '2.5rem' }}>
                ...
                <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', marginBottom: '1.5rem' }}>
                  ...
                  <div style={{ flex: '1', minWidth: '250px' }}>
                    ...
                    <select
                      value={expandedAgent || ''}
                      onChange={(e) => setExpandedAgent(e.target.value as AgentType || null)}
                      style={{
                        width: '100%',
                        padding: '0.75rem 1rem',
                        borderRadius: '8px',
                    ...
                    >
                      ...
                      {(Object.keys(agentInfo) as AgentType[]).map(key => (
                        <option key={key} value={key}>
                          {agentInfo[key].icon} {agentInfo[key].nameRu}
                        </option>
                      ))}
                    </select>
                  </div>
                ...
                {expandedAgent && (
                  <div style={{
                    backgroundColor: 'var(--bg-secondary)',
                    borderRadius: '12px',
                    border: `2px solid ${agentStates[expandedAgent].status === 'running' ? 'var(--accent-color)' : agentStates[expandedAgent].status === 'complete' ? '#22c55e' : 'var(--border-color)'}`,
                    overflow: 'hidden'
                  }}>
                    {/* Agent Header */}
                    <div style={{
                      ...
                      <span style={{ fontSize: '2rem' }}>{agentInfo[expandedAgent].icon}</span>
                      <div style={{ flex: 1 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.25rem' }}>
                          <h3 style={{ margin: 0, fontSize: '1.1rem', fontWeight: '600' }}>{agentInfo[expandedAgent].nameRu}</h3>
                          {agentStates[expandedAgent].status === 'running' && (
                            <span style={{
                              fontSize: '0.75rem',
                              padding: '3px 8px',
                              backgroundColor: 'var(--accent-color)',
                          ...
                          {agentStates[expandedAgent].status === 'complete' && (
                            <span style={{
                              fontSize: '0.75rem',
                              padding: '3px 8px',
                        ...
                        <span style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>{agentInfo[expandedAgent].name}</span>
                      </div>
                    </div>

                    {/* Agent Description */}
                    <div style={{ padding: '1rem 1.25rem', backgroundColor: 'var(--bg-primary)' }}>
                      <p style={{
                        fontSize: '0.9rem',
                        color: 'var(--text-secondary)',
                        lineHeight: '1.5',
                        margin: 0
                      }}>{agentInfo[expandedAgent].description}</p>
                    </div>
                    ...
                    <div style={{ padding: '1.25rem' }}>
                      ...
                      <div style={{ marginBottom: '1rem' }}>
                        ...
                        <textarea
                          value={agentStates[expandedAgent].prompt}
                          onChange={(e) => updateAgentState(expandedAgent, { prompt: e.target.value })}
                          placeholder="Введите ваш запрос или дополнительные инструкции для агента..."
                          disabled={agentStates[expandedAgent].status === 'running'}
                          style={{
                            width: '100%',
                            minHeight: '100px',
                            padding: '0.875rem',
                      ...
                      {/* Run Button */}
                      <button
                        onClick={() => runAgent(expandedAgent)}
                        disabled={agentStates[expandedAgent].status === 'running' || !selectedProjectForAgents}
                        style={{
                          width: '100%',
                          padding: '0.875rem',
                          borderRadius: '8px',
                          border: 'none',
                          backgroundColor: agentStates[expandedAgent].status === 'running' ? 'var(--border-color)' : !selectedProjectForAgents ? 'var(--border-color)' : 'var(--accent-color)',
                          color: 'white',
                          fontSize: '1rem',
                          fontWeight: '600',
                          cursor: agentStates[expandedAgent].status === 'running' || !selectedProjectForAgents ? 'not-allowed' : 'pointer',
                          display: 'flex',
                          alignItems: 'center',
                      ...
                      >
                        {agentStates[expandedAgent].status === 'running' ? (
                          <>⏳ Анализ выполняется...</>
                        ) : !selectedProjectForAgents ? (
                          <>⚠️ Сначала выберите проект</>
                        ) : (
                          <>▶️ Запустить {agentInfo[expandedAgent].nameRu}</>
                        )}
                      </button>

                      {/* Error Display */}
                      {agentStates[expandedAgent].error && (
                        <div style={{
                          marginTop: '1rem',
                          padding: '1rem',
                          ...
                          <strong>Ошибка:</strong> {agentStates[expandedAgent].error}
                        </div>
                      )}

                      {/* Results Display */}
                      {agentStates[expandedAgent].result && (
                        <div style={{
                          marginTop: '1rem',
                          padding: '1rem',
                          ...
                          <div style={{ fontWeight: '600', marginBottom: '0.75rem', color: '#22c55e', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                            ✅ Результат анализа:
                          </div>
                          <pre style={{
                            whiteSpace: 'pre-wrap',
                            ...
                            lineHeight: '1.6',
                            color: 'var(--text-primary)'
                          }}>{agentStates[expandedAgent].result}</pre>
                        </div>
                      )}
                    </div>
                  </div>
                ...
                {!expandedAgent && (
                  <div style={{
                    ...
                    <p style={{ color: 'var(--text-secondary)', margin: 0, fontSize: '0.95rem' }}>
                      Выберите AI-агента из списка выше для анализа тендерной документации
                    </p>
                  </div>
                )}
              </div>
